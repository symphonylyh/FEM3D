...
"""
Automate mesh generation for 3D FEM code.

Example:
    $ python mesh_generator.py -f
"""

import numpy as np
import argparse

parser = argparse.ArgumentParser(description='Automate mesh generation for 3D FEM code.')
parser.add_argument("-f", "--filename", help="file name")
parser.add_argument("-m", "--model", type = int, help="model name")
...
def read_model(file):
    ...
    for i in range(4):
        ...
        model_para.append(temp)
    return model_num, model_para
...
model_num_list, model_para_list = read_model(file)
model_num = model_num_list[model_name - 1]
model_para = model_para_list[model_name - 1]

for file in range(len(sigma3)):
    name = "input_" + str(file).zfill(2) + ".txt"
    if (Test_method == 'Test'):
        full_surface_load = 1
    # ========================================================================================#
        x_len = 75 * 1 / 25.4
